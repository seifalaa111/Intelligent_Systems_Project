...
from midan.intelligent_score import compute_intelligent_score  # noqa: F401
from midan.rag import compute_shap_cosine, query_explicit_rag  # noqa: F401
from midan.react_router import react_route  # noqa: F401
...
def _build_signal_consensus_summary(
    regime: str,
    ...
    """
    Deterministic 6-rule hierarchy producing one explanation-coherent string.
    Resolves contradictions between IS, SHAP, RAG, and L3 before any text
    surface reads these values — prevents the explanation layer from generating
    conflicting statements about the same underlying signals.

    Rule priority (first match wins):
      1. force_human_review → escalation message
      2. is_novel_case      → novelty framing
      3. PATH_1/PATH_2 with no conflicts → clean agreement message
      4. PATH_3             → borderline confirmed by precedent
      5. PATH_4/PATH_5/PATH_6 → named disagreement
      6. Default            → weak alignment
    """
    from midan.react_router import (
        PATH_1_HIGH_CERTAINTY, PATH_2_LOW_CERTAINTY,
        PATH_3_BORDERLINE_CONFIRMED, PATH_4_BORDERLINE_CONFLICT,
        PATH_5_ATYPICAL_SUPPORTED, PATH_6_FULL_CONFLICT,
    )

    path_id      = react_decision.get('path_id', '')
    force_review = react_decision.get('force_human_review', False)
    is_novel     = react_decision.get('is_novel_case', False)
    rag_vote     = react_decision.get('rag_vote')
    conflicts    = l3_reasoning.get('signal_interactions', [])
    n_conflicts  = len([c for c in conflicts if c.get('type') == 'conflict'])

    # Rule 1 — escalation
    if force_review:
        return (
            f"Signals insufficient for automated decision (IS={intelligent_score:.2f}, "
            f"path={path_id}). Human review required before acting."
        )

    # Rule 2 — novelty
    if is_novel:
        return (
            f"Novel case: no structural precedent in training space "
            f"(IS={intelligent_score:.2f}, regime={regime}). "
            "Analysis proceeds on macro fundamentals only — precedent-based confidence unavailable."
        )

    # Rule 3 — clean high/low certainty
    if path_id in (PATH_1_HIGH_CERTAINTY, PATH_2_LOW_CERTAINTY) and n_conflicts == 0:
        direction = "favorable" if path_id == PATH_1_HIGH_CERTAINTY else "unfavorable"
        return (
            f"High signal agreement: IS={intelligent_score:.2f} ({direction}), "
            f"attribution typical, regime={regime}. "
            "All three geometric signals consistent."
        )

    # Rule 4 — borderline confirmed
    if path_id == PATH_3_BORDERLINE_CONFIRMED:
        return (
            f"Borderline IS={intelligent_score:.2f} confirmed by precedent analysis "
            f"(RAG vote aligns with SVM regime={regime}). "
            "Proceed with moderate confidence."
        )

    # Rule 5 — named disagreement
    if path_id == PATH_4_BORDERLINE_CONFLICT:
        return (
            f"Borderline IS={intelligent_score:.2f}: RAG precedent "
            f"({rag_vote or 'unknown'}) disagrees with SVM regime ({regime}). "
            "Decision rests on regime classification; treat as higher-uncertainty."
        )
    if path_id == PATH_5_ATYPICAL_SUPPORTED:
        return (
            f"Attribution pattern atypical for regime={regime} "
            f"(shap_cosine={is_result['components']['shap_cosine']:.2f}), "
            f"but RAG precedent ({rag_vote or 'n/a'}) supports SVM classification. "
            "Proceed with reduced attribution confidence."
        )
    if path_id == PATH_6_FULL_CONFLICT:
        return (
            f"Full conflict: attribution atypical AND RAG ({rag_vote or 'unknown'}) "
            f"disagrees with SVM regime ({regime}). "
            "Two independent doubt signals active — findings should not drive commitment decisions."
        )

    # Rule 6 — default weak alignment
    return (
        f"Signals weakly aligned: IS={intelligent_score:.2f}, regime={regime}, "
        f"path={path_id}. Treat conclusions as indicative, not definitive."
    )
...
def run_inference(
    ...
    # ── L2C.5: Implicit RAG — SHAP attribution consistency ───────────────────
    top_cluster_idx = fcm_signal.get('top_cluster') if fcm_signal.get('available') else None
    shap_cosine = compute_shap_cosine(
        shap_dict=shap_dict,
        shap_cluster_means=shap_cluster_means,
        top_cluster_idx=top_cluster_idx,
        feature_order=FEATURE_ORDER,
    )
    logs.append(f"[L2C.5] shap_cosine={shap_cosine:.3f} (attribution consistency vs cluster mean)")
    ...
    intelligent_score = is_result['score']
    logs.append(
        f"[L2D.5] IS={intelligent_score:.3f} | gap_svm={is_result['gap_svm']:.3f} | "
        f"trio_discount={is_result['is_correlated_trio']} | "
        f"components=s={is_result['components']['s_favorable']:.2f},"
        f"gap={is_result['components']['gap_svm']:.2f},"
        f"mu={is_result['components']['mu_fcm']:.2f},"
        f"arima={is_result['components']['arima']:.2f},"
        f"shap={is_result['components']['shap_cosine']:.2f}"
    )
    ...
    signal_consensus = _build_signal_consensus_summary(
        regime=regime,
        intelligent_score=intelligent_score,
    ...
    logs.append(
        f"[REACT] path={react_decision['path_id']} | IS={intelligent_score:.3f} | "
        f"force_review={react_decision['force_human_review']} | "
        f"rag_vote={react_decision.get('rag_vote')} | rag_skipped={rag_result.get('rag_skipped')} | "
        f"novelty={react_decision['is_novel_case']} | asymmetric={react_decision['asymmetric_opportunity']}"
    )
    ...
    log_prediction({
        ...
        'request_id':        id(idea_text),
        'sector':            sec,
        'country':           country,
        'regime':            regime,
        'gap_svm':           is_result['gap_svm'],
        'intelligent_score': intelligent_score,
        'shap_cosine':       shap_cosine,
        'mu_fcm':            mu_fcm_val,
        'sarima_trend':      sarima_trend,
        'x_pca':             list(x_pca[0]),
        'novelty_score':     rag_result.get('novelty_score', 0.0),
        'react_path':        react_decision['path_id'],
        'l4_decision_state': l4_decision['decision_state'],
    })
    ...
    return {
        ...
        'mechanism_analysis': serialize_envelope(mechanism_envelope),
        # ── ReAct routing surface ─────────────────────────────────────────
        'intelligent_score':    intelligent_score,
        'is_components':        is_result['components'],
        'is_correlated_trio':   is_result['is_correlated_trio'],
        'react_path':           react_decision['path_id'],
        'react_decision':       react_decision,
        'rag_result':           rag_result,
        'shap_cosine':          shap_cosine,
