...
from midan.config import (
    ...
    RETRAIN_ROLLBACK_ACCURACY_FLOOR,
...
def _pkl_optional(name):
    """Load artifact or return None. All new RAG/IS components use this so
    the system degrades gracefully when artifacts have not been generated yet."""
    try:
        with open(f'{MODELS_DIR}/{name}', 'rb') as f:
            return pickle.load(f)
    except Exception as _pkl_opt_err:
        _CORE_LOG.info(
            "[CORE] _pkl_optional('%s') absent or unreadable (%s) — degraded mode active",
            name, type(_pkl_opt_err).__name__,
        )
        return None
...
#   regime_anchors_pca  absent → outer retraining loop hard-aborts (unsafe to retrain
#                                 without semantic anchors)
shap_cluster_means = _pkl_optional('shap_cluster_means.pkl')
rag_index          = _pkl_optional('rag_index.pkl')
rag_labels         = _json('rag_labels.json', [])
