...
def compute_l4_decision(
    ...
    react_decision: Optional[dict] = None,
) -> dict:
    """
    ...
    if react_decision and react_decision.get('rag_conflict_severity') in ('low', 'medium'):
        ...
        conflicts.append({
            'conflict_id':  'rag_vs_svm_regime',
            'severity':     react_decision['rag_conflict_severity'],
            'description':  (
                f"RAG k-NN precedent vote ({rag_vote_label}) disagrees with "
                f"SVM regime ({regime}). Conflict surfaced by react_router "
                f"path {react_decision.get('path_id', '?')}."
            ),
            'source':       'react_router',
            'key_signals':  {
                'rag_vote':       rag_vote_label,
                'svm_regime':     regime,
                'conflict_level': react_decision['rag_conflict_severity'],
            },
        })
    ...
    if react_decision and react_decision.get('force_human_review'):
        if sm['state'] not in ('INSUFFICIENT_DATA',):
            ...
            sm['state_reasoning'] = (
                f"[react_router override] {sm['state_reasoning']} "
                f"| ReAct path {react_decision.get('path_id', '?')} "
